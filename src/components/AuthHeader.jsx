...
const AuthHeader = () => {
  const { session,logout,certificateReq,setcertificateReq } = useAuth();
  return (
    ...
    <div className="d-flex justify-content-between align-items-center w-100">
      ...
      <nav id="navmenu" className="navmenu" >
        ...
        {<li>{session?.user?.role == 'franchise'? <Link to='/register-student'>Enroll Student</Link>:null}</li> }
          {/* {<li>{session?.user?.role == 'admin'? <Link to='/add-instructor'>Instructors</Link>:null}</li> } */}
        {<li>{session?.user?.role == 'admin'?
         <Link to='/certificate-req' className='position-relative'>
         Certificate Request
         {certificateReq?.students?.length > 0 && <span className='position-absolute top-0 end-0 bg-danger text-light px-2' style={{fontSize:'14px',borderRadius:'50%'}}>{certificateReq?.students?.length}</span>}
        </Link>:null}
        </li> }
